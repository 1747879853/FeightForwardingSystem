...
| 关键源码 | `src/router/routes/modules/freight-rate.ts`<br/>`src/views/freight-rate/list.vue`<br/>`src/views/freight-rate/batch-add-page.vue`<br/>`src/views/freight-rate/pending-batch-rows.ts`<br/>`src/views/freight-rate/data.ts`<br/>`src/api/sea-export/freight-rate-admin.ts` |
...
- **列配置持久化：** 列表与批量新增/编辑 Tab 页表格各自在 `gridOptions.id` 声明独立 id，列显隐/顺序/固定/列宽互不覆盖。
