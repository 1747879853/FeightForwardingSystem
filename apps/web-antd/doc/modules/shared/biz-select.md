...
- **指定用户所属组织（`UserOrgSelect`）：** 「先选人、再选该人所属组织」场景（如海出选定销售后选 `orgId`）；数据来自 `GetAllUserOrganizationsAsync`（`use-all-user-org`）。已有快照时立刻展示，选销售、打开下拉或会话 reset 后会后台静默重拉；默认填该用户默认组织，越范围旧值自动清空。登录/登出只清空 map，不预拉。
...
| **orgId (UserOrgSelect)** | 指定用户（如销售）的归属组织 id | `UserAdmin/GetAllUserOrganizationsAsync` → `use-all-user-org` | 依赖 `userId`；换人/打开下拉静默刷新 map；可经 `getUserOrgCompanyNode(userId, orgId)` 取公司节点 | 须先有 `userId`；值为该用户直属组织 id |
...
| :-- | :-- | :-- | :-- |
| 2026-09-22 | `Fix` | 账号新绑公司后，选销售或打开归属组织下拉会刷出新抬头，不必整页刷新。 | `loadAllUserOrganizations` 改为有快照则静默重拉；登录只 reset。详见 `changelogs/change-log-2026-09-22-归属组织下拉静默刷新.md`。 |
