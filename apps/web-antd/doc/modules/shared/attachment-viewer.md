...
**白话解释：** 业务人员在任意页面点附件，都在同一个弹窗里预览：图片和 PDF 直接看，Word/Excel/PPT 由浏览器内的 vue-office 渲染。相对路径会拼当前品牌后端，Office 文件经同源 `/Uploads` 拉取，不依赖微软在线预览。
...
- **地址补全：** 下载仍拼后端根；预览 fetch / PDF iframe 走 `resolveSameOriginMediaUrl`（`/Uploads/...`），开发走 Vite 代理。
...
| :-- | :-- | :-- | :-- | :-- |
| **url / fileUrl** | 附件访问地址，相对或绝对均可 | 调用方传入（附件 DTO 或字符串） | 触发弹窗打开；Office/PDF 预览再改写为同源 `/Uploads` | 空则提示「附件链接不存在」 |
...
| **后端根** | 附件下载直连的后端 origin | `VITE_GLOB_STATIC_URL` / `VITE_GLOB_API_URL` 去 `/api` | 预览不走微软 | 开发环境 `/Uploads` 必须能代理到后端 |
