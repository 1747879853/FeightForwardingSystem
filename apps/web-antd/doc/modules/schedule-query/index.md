...
- **前端二次筛选：** 直达/中转、船公司、起运/目的标准码头、船名/航次/航线/中转港关键词和方案排序都作用于已拉取的全量结果，不重复请求实时接口。船公司与码头多选后完整展示已选项，不收成 `+N`。默认按周一至周日排方案。
...
- **班次比较与完整详情：** 方案卡主标题是共舱名，左侧直达/中转 Tag；下面是星期短写、班次数、码头；右侧航程、最近离港、最早截关。展开后用原生表格对照船名航次、计划离到港、截关、航程；延误写「延误 N 天」。点击共舱名复制。点船名或「详情」打开船期字段弹窗（船舶航线、港口码头、中转路径、时间航程、截点、数据标识）。船舶定位已暂时下线。
...
| **mmsi / vessel（AIS 定位）** | 班次上的 MMSI / 船名。船舶定位已暂时下线，字段仍随详情展示。 | 查询结果行数据（`QueryScheduleAsync`） | 点船名 / 详情只打开字段弹窗。 | 选读。 |
| **groupName** | 方案身份：共舱 `船公司(航线)` 去重字母序拼接；无共舱则本班次承运人+航线。 | 前端由 `shareCabins` / `carrierCd`+`routeCode` 生成 | 与直达/中转一起构成分组键；卡片主标题。 | 有共舱时不用本班次航线代码，避免脏串拆组。 |
| **routeEtd** | 计划离港班期（SUN–SAT）。 | 查询结果行数据 | 卡片列出组内全部星期短写，**不拆组**；方案排序仍用众数。 | 选读。 |
...
> [!IMPORTANT] **[卡点 2：方案接口未对接]** 本系统没有港到港方案分组接口，分组由扁平班次在前端重建。青岛→新加坡 8 周对照总数同为 62，直达/中转会差 1：飞驼中转 `CNC` 是 8 条无船名的卡车班次，扁平接口没有；本地直达多 `WHL(AA1)`（万海 52 天），飞驼方案列表未挂出。超过 1000 条时还受后端分页上限影响。
