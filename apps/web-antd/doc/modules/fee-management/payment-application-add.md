...
- **发票明细子表：** 发票方式下方为可增删行（发票号、开票日期、销售方抬头、发票金额、每行一个附件），底部展示发票总额（前端对已填金额求和）。「从进项发票选择」在发票方式 Tab「不开票」右侧（不开票时隐藏），勾选关联进项票并回填；已填票号（含未保存）会作为 `excludeInvoiceNos` 排除，结算币别已选银行时传该银行所属 `clientInvoiceInfoId`。先票后付可先空着保存，提交前必须至少一条且发票号必填；不开票禁用并清空；先付后票可空。发票行附件走 `UploadAndExtractInvoiceAsync`（PDF/图片），上传成功即回填该行票号、日期、抬头与金额；识别失败附件仍保留。仍可用「重新识别」对已有附件再调 `ExtractInvoiceAsync`。识别结果与进项勾选都不自动保存。申请右侧 `attachmentGroup` 仍是付费申请自身附件，与发票行附件模块不同，上传不识别、不回填发票行。
- **申请附件分组：** 右侧附件区按附件明细类型分组上传；支持点击或拖拽；先通用上传得 `attachmentId`，新建随 `AddAsync.attachmentGroup` 一并绑定。附件卡片高度随左侧结算/发票/币别内容撑开，铺满审核流程下方剩余空间（最低 200px）。关联结算附件不在本页维护。
...
| **开船日期** | 添加费用抽屉外层列：业务开船日。 | 分组 `etd` | **触发/依赖：** 只读；页内费用明细原已有此列。 | 仅展示年月日（`YYYY-MM-DD`）。 |
...
| **对账单号（检索）** | 按客户对账单号模糊筛选可申请费用。 | `GetOrderFeeGroupAsync.StatementNum` | **触发/依赖：** trim 后有值才传；与已审核、剩余可申请金额 > 0 等原条件是与关系。 | 空值不传，避免空串误过滤。 |
| **对账单号（明细列）** | 该费用进过的对账单号，多张中文逗号拼接。 | 费用 `statements[].statementNum` → 行上 `statementNums` | **触发/依赖：** 只读展示；未对账为空。 | 展示全部对账单，不被检索条件裁剪。 |
...
| **发票明细** | 一单可多张发票，每张可挂一个附件。 | `paymentApplicationInvoices[]` → `AddAsync`；上传识别走 `GeminiAdmin/UploadAndExtractInvoiceAsync`；挑票走 `PaymentApplicationAdmin/GetInputInvoiceListAsync` | **触发/依赖：** 「从进项发票选择」回填票号/日期/抬头/金额/PDF；`orgId` 传公司节点；已填票号进 `excludeInvoiceNos`；结算币别已选银行时传 `clientInvoiceInfoId`。发票**行**上传 PDF/图片即回填当前行；申请附件分组上传不识别、不回填。不开票时清空。先票后付提示「可先保存，提交前再补录」。底部总额前端求和。 | 录入阶段先票后付可空；提交时至少一条；不开票必须空；发票号必填、最长 128、同一申请不可重复。抬头最长 256；金额可空、允许负数，不与申请金额比对。发票行只支持 PDF/图片。 |
...
> [!IMPORTANT] **[卡点 3：发票方式必选，先票后付发票只卡提交]** 未选定 `invoiceProcess` 时，添加费用确认、应收应付预填后保存、新建保存均拦截（toast「请选择发票方式」），不会调用 `AddAsync`。先票后付空发票可以建单；点提交才提示「提交前必须录入发票信息」。不开票仍当场清空发票行。

> [!IMPORTANT] **[卡点 4：新建保存后必须关闭原 Tab]** `/add` 与 `/:id/edit` 是不同 Tab key；仅 `replace` 仍会留下新建页签。须先缓存 `route.fullPath`，`await replace` 后再 `closeTabByKey`。

> [!IMPORTANT] **[卡点 5：上传发票即识别，不自动保存]** 仅发票**行**附件走 `UploadAndExtractInvoiceAsync`，只收 PDF/图片；识别失败 `invoice` 为 null 但附件已保留。申请附件分组上传不识别、不回填发票行。未点页面「保存」刷新后丢失。超时须 180 秒。

> [!IMPORTANT] **[卡点 6：从进项发票选择]** `GetInputInvoiceListAsync` 的 `orgId` 必须是公司 id，传部门会空列表且不报错。已填未保存的票号必须进 `excludeInvoiceNos`。结算币别已选银行时传该银行所属 `clientInvoiceInfoId`，多币别优先人民币（`RMB`/`CNY`）；否则用结算对象全部开票信息税号。勾选不自动保存。
...
| 2026-09-07 | `Feature` | 上传发票附件后自动识别回填票号、日期、抬头与金额，不必再点识别。 | `UploadAndExtractInvoiceAsync`；`invoice` 为 null 时附件仍保留。详见 `changelogs/change-log-2026-09-07-payment-application-invoice-auto-extract.md`。 |
...
| 2026-08-28 | `Feature` | 支持从海出/海进/空出应收应付带 `orderFeeIds` 预填明细并跳过自动开抽屉；状态校验与表格展示口径对齐。 | `open-from-order-fees.ts` + `prefill-from-order-fee-ids.ts`；详见 `changelogs/change-log-2026-08-28-order-fee-create-payment-application.md`。 |
