...
| **发票明细** | 一单可多张发票，每张可挂一个附件。 | `paymentApplicationInvoices[]` → `AddAsync` | **触发/依赖：** 识别结果写入第一张空发票号的行，没空行则追加；不开票时清空。先票后付提示「可先保存，提交前再补录」。底部总额前端求和。 | 录入阶段先票后付可空；提交时至少一条；不开票必须空；发票号必填、最长 128、同一申请不可重复。抬头最长 256；金额可空、允许负数，不与申请金额比对。 |
