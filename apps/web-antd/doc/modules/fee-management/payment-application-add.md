...
- **发票明细子表：** 发票方式下方为可增删行（发票号、开票日期、销售方抬头、发票金额、每行一个附件），底部展示发票总额（前端对已填金额求和）。可点「从进项发票选择」勾选关联进项票并回填；已填票号（含未保存）会作为 `excludeInvoiceNos` 排除，结算币别已选银行时传该银行所属 `clientInvoiceInfoId`。先票后付可先空着保存，提交前必须至少一条且发票号必填；不开票禁用并清空；先付后票可空。发票行附件走 `UploadAndExtractInvoiceAsync`（PDF/图片），上传成功即回填该行票号、日期、抬头与金额；识别失败附件仍保留。仍可用「重新识别」对已有附件再调 `ExtractInvoiceAsync`。识别结果与进项勾选都不自动保存。申请右侧 `attachmentGroup` 仍是付费申请自身附件，与发票行附件模块不同，上传不识别、不回填发票行。
