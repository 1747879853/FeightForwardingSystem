...
| **申请汇率** | 指定结算币别时，1 原币兑多少结算币。 | 弹窗预填：`peekExchangeRate`（公司本位币 + 开船日/今天 + 应付 `crValue`）交叉相除；可手改 | **触发/依赖：** 添加费用确认时弹出「币别汇率折算」；同原币不同 ETD 分组；写入费用 `exchangeRate`/`rate`。 | 必须 >0；无有效汇率时不预填，须手填；勿用 RMB/CNY 代码当本位币。 |
