...
| **发票明细** | 一单可多张发票，每张可挂一个附件。 | `paymentApplicationInvoices[]` → `AddAsync`；上传识别走 `GeminiAdmin/UploadAndExtractInvoiceAsync`；挑票走 `PaymentApplicationAdmin/GetInputInvoiceListAsync` | **触发/依赖：** 「从进项发票选择」回填票号/日期/抬头/金额/PDF；`orgId` 传公司节点；已填票号进 `excludeInvoiceNos`；结算币别已选银行时传 `clientInvoiceInfoId`。上传 PDF/图片即回填当前行；申请附件发票分组回填第一张空发票号的行，没空行则追加；不开票时清空。先票后付提示「可先保存，提交前再补录」。底部总额前端求和。 | 录入阶段先票后付可空；提交时至少一条；不开票必须空；发票号必填、最长 128、同一申请不可重复。抬头最长 256；金额可空、允许负数，不与申请金额比对。只支持 PDF/图片。 |
...
> [!IMPORTANT] **[卡点 5：上传发票即识别，不自动保存]** 发票行和发票分组附件走 `UploadAndExtractInvoiceAsync`，只收 PDF/图片；识别失败 `invoice` 为 null 但附件已落。未点页面「保存」刷新后丢失。超时须 180 秒。
