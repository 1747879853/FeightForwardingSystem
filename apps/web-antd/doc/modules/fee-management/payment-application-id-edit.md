...
| **发票明细** | 一单可多张发票，每张可挂一个附件。 | 详情 `paymentApplicationInvoices[]`；识别走 `GeminiAdmin/ExtractInvoiceAsync` | **触发/依赖：** 编辑全量覆盖；识别结果写入当前行或第一张空行，不自动保存；不开票时清空。 | 先票后付至少一条；不开票必须空；发票号必填、最长 128、同一申请不可重复。 |
