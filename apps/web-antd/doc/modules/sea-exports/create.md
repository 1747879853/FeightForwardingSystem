...
- **服务项目联动：** 选择「起运港」（`polId`）后会先调用 `GetServiceTypesByPOLAsync`（仅传 `polId`）确定可见服务卡片及顺序；再按 `polId + clientId` 查询默认勾选。未选或清空起运港时不渲染服务节点，仅展示「请先选择起运港」；起运港已选但未配置任何服务项时展示「该起运港未配置服务项目」。
