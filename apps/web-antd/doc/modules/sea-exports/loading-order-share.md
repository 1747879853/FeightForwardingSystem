...
- **监装直播：** 进入页面先静默调 `LoadingOrderVideo/GetViewerCountAsync` 探活，不点播。探活成功才渲染「查看监装视频」；失败（如未绑定摄像头）只展示接口报错原文，不露按钮。已完成仍只提示已关闭，不探活。点击后全屏打开再调 `StartPlayAsync`，点播超时 120 秒；mpegts.js 只播返回的 flvUrl，首帧最多等 30 秒。异常先断开再点播一次查业务原因，由用户手动重试。连接数每 10 秒查一次，失败显示未知。云台在全屏画面右下角，按住方向或变倍、松开停止；Esc 或关闭销毁播放器并释放观看位。视频及云台请求免登录，不带 token。
...
| 2026-09-13 | `Fix` | 分享页收窄并按运输、货物、监装分组，明确工单和箱状态，补充照片空态说明。 | 保持公开接口与直播点播逻辑，中英文及手机布局同步。 | | 2026-09-13 | `Fix` | 未绑摄像头时不展示「查看监装视频」，先静默探活；失败直接渲染接口报错。 | 探活用 `GetViewerCountAsync`，避免 `StartPlayAsync` 提前起流占观看位。 |
