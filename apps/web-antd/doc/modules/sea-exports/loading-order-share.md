...
- **监装直播：** 公开详情 `cameraNo` 为空则只提示「暂无现场视频」，鼠标悬停说明未绑定摄像头，不调点播。有摄像头才渲染「查看监装视频」。点击后全屏打开再调 `StartPlayAsync`，点播超时 120 秒；失败展示接口业务文案（ABP `error.message`），不是通用「连接失败」。mpegts.js 只播返回的 flvUrl，首帧最多等 30 秒。异常先断开再点播一次查业务原因，由用户手动重试。已完成只提示已关闭。连接数每 10 秒查一次，失败显示未知。云台在全屏画面右下角，按住方向、松开停止；放大/缩小变倍按钮暂不展示（`showZoom = false`，改 true 可恢复）。Esc 或关闭销毁播放器并释放观看位。视频及云台请求免登录，不带 token。
...
- **打开公开页：** 访问 `/loading-order-share?mblNum=&loadingOrderNum=`（hash 路由品牌实际为 `/#/loading-order-share?...`）。可选 `lang=en` 展示英文界面，缺省中文；文案跟 URL 语言走，不跟操作系统或后台账号语言。页头品牌 Logo + 标题固定在顶。主体是一张按内容高度收缩的单据，最宽 1360px：主提单号与蓝色「查看监装视频」独立置顶，下方按运输、货物、监装三组展示有值字段，800px 以下分组纵向排列；空字段不展示，监装组始终保留工单与状态。箱状态紧跟箱号与封号，无照片时说明由现场人员上传后展示。集装箱接在下面，不左右分栏、不拉满视口。箱照按类型横排。底栏「品牌 · 监装信息共享」贴在窗口底部，内容超出时只滚中间。
...
| 2026-09-13 | `Fix` | 分享页收窄并按运输、货物、监装分组，明确工单和箱状态，补充照片空态说明。 | 保持公开接口与直播点播逻辑，中英文及手机布局同步。 | | 2026-09-13 | `Fix` | `cameraNo` 为空只提示暂无视频，悬停说明未绑定；点播失败展示 `StartPlayAsync` 业务文案。 | 请求层会把 Axios 错误收成 ABP 响应体，必须读 `error.message`。 |
