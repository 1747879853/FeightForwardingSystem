...
- **打开公开页：** 访问 `/loading-order-share?mblNum=&loadingOrderNum=`（hash 路由品牌实际为 `/#/loading-order-share?...`）。可选 `lang=en` 展示英文界面，缺省中文；文案跟 URL 语言走，不跟操作系统或后台账号语言。页头品牌 Logo + 标题滚动吸顶。概览区展示主提单号、工单号与工单状态。主体为基本信息卡与集装箱卡（含分组照片预览）。
