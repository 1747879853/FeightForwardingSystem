...
- **分页检索：** `createPagedListQuery(getLoadingOrderPagedList, { defaultSort: 'EstimatedArrivalTime DESC', fieldMap: LOADING_ORDER_SORT_FIELD_MAP, mapParams: normalizeQuery })` 调 `LoadingOrderAdmin/GetPagedListAsync`。搜索不自动查询（`submitOnChange: false`），需点「查询」。默认按预计到货时间倒序，与小程序列表一致。
