...
>
> **[卡点 15：监装照片不是工单编辑]** 管理端改某箱监装照片走 `EditOrderCtnAttachmentGroupsAsync`，`id` 是箱型 id 不是工单 id；不要塞箱号/封号/是否完成，也不要再调师傅端 `EditOrderCtnsAsync`（那条仍要求已认领且会改箱子字段）。弹窗先按 `160100` 铺维护类型，再叠已有照片；附件按该箱全量替换，漏传某组等于删掉该组照片。每分类可多选追加，批量上传失败保留成功图片；上传中不可关闭或保存。仅新上传图片写入水印，历史图片保持原样；水印时间为设备时间转换后的北京时间。
>
...
| 2026-09-18 | `Feature` | 监装每分类支持多图，批量上传，新增上传人与北京时间水印。 | 串行队列完成前禁止保存；附件分组仍全量替换，水印固化为 JPEG。详见 [变更记录](../../changelogs/change-log-2026-09-18-监装分类多图与上传水印.md)。 |
