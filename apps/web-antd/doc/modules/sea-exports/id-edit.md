...
>
> **[卡点 14：监装照片不是工单编辑]** 管理端改某箱监装照片走 `EditOrderCtnAttachmentGroupsAsync`，`id` 是箱型 id 不是工单 id；不要塞箱号/封号/是否完成，也不要再调师傅端 `EditOrderCtnsAsync`（那条仍要求已认领且会改箱子字段）。弹窗先按 `160100` 铺维护类型，再叠已有照片；附件按该箱全量替换，漏传某组等于删掉该组照片。类型横排、每类型限 1 张；换图先删再传，历史多图不会在保存时自动截断。
>
