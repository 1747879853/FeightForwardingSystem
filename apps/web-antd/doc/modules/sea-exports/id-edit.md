...
| 2026-07-13 | `Fix` | 运踪里程碑与集装箱轨迹取消前端排序，完全按 `GetOceanPushInfoAsync` 返回顺序展示。 | `yundang-tracking-panel.vue` 移除 `oceanNodes` 的 `actualityTime` 排序与 `sortContainerStatuses`；箱轨迹末项仍标「进行中」。 |
