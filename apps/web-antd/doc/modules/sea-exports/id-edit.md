...
- **订单费用处理：** 费用页基于运输单 ID 加载应收应付费用；Handsontable 左侧为勾选、独立序号、开票状态（不再把行号画在开票状态格里），其后为费用代码、结算对象、币种、汇率、单价、金额、税率、开票/结算金额、可开票、机密、费用状态等；费用状态包括录入、提交审核、审核通过、驳回、申请修改、申请删除、部分结算、结算完毕。改币别时汇率只取「费用币别兑所属公司本位币」且业务日期（海出=开船日）有效的记录，对不上留空手填；费用币别本身就是本位币则锁 1。工具栏新增「**创建付费申请**」（需 `Admin.PaymentApplication.Add`）：勾选应付且组合状态为审核通过/部分结算、同一结算对象、跳转前先 `GetOrderFeeGroupAsync` 回捞，成功后进入付费申请新增页预填（`orderFeeIds` query）；先票后付可先建单，提交时才要求至少一条发票。应收/应付表工具栏「打印」：`printJsonType` 分别为 `1000`（应收）/ `1500`（应付），拉模板列表传 `bizType=0`（海运出口，结果含通用模板）；由后端按 `transportOrderId` 取数，勾选已保存费用时传 `orderFeeListInput.ids` 仅打勾选项，未勾选则打整票。**完整功能说明见** [应收应付费用录入](../shared/order-fee-entry.md)。
...
- **监装工单：** Tab 文案为「监装」，位于派车与分单之间；需 `Admin.SeaExport.LoadingOrder.Get`，无权限时整 Tab 不出现、也不参与 Tab 记忆恢复。进入即调 `LoadingOrderAdmin/DetailBySeaExportIdAsync`（**传海出 id**），返回 `null` 且有 `.Add` 时直接进入新建表单（不先点「新建」）；无 `.Add` 才显示空态。版式按 Figma 节点几何：工号只在基础信息第一格（带复制），顶栏为状态 +「分享 / 保存 / 提交 / 删除」；已保存工单可点「分享」先弹出客户页预览，可切中英文后再复制免登录链接（主提单号 + 监装工单号，英文带 `lang=en`，`router.resolve` 兼容 hash），客户打开 `/loading-order-share` 只看基本信息与箱照、不看监装要求与备注。五列栅格（列间距 13px、行间距 12px），控件用默认 middle 高度；第三行预计到货/堆场/师傅各占一列，师傅为多选（人数不限）；堆场标签旁橙色「推荐」可点：先选预计到货时间且已保存船公司，弹窗调 `GetYardUsersAsync`，单选一行后回填堆场与师傅（接口只回名称，前端对本地 id）；监装要求已选标签区常驻一行高度，避免 0→1 勾选抖动；详细说明读写工单 `remark`（高 62px，最长 1024，未提交可改）；集装箱 32px 浅蓝标题条，列表头 36/行 50，照片采集按钮 123×30 用稿面相机 SVG。有 `.Edit` 时点按钮打开分组编辑弹窗：先拉 `GetListByModuleTypesAsync(160100)` 铺类型空槽再填已有照片，空箱箱内置顶独占整行并支持多选及持续追加，其余类型按原始 `sortId` 降序并排展示、空间不足换行，每类限一张，有图隐藏添加入口；历史多图保留，每类显示张数；PC 新上传图片仅固化上传人和北京时间水印，不获取位置，保存调 `LoadingOrderAdmin/EditOrderCtnAttachmentGroupsAsync`（`id`=箱型 id，只改该箱监装附件，任意状态可调，全量替换）；无编辑权限则只预览。**明细包装与堆场的候选项只认已保存的海运出口**（基础信息未点保存不会带到监装；切回监装 Tab 会重拉堆场）。无船公司与船公司未维护堆场时下拉不禁用、打开为空列表，用不同空态文案。明细包装无候选项时仍禁用：未填包装用「该海运出口未填写包装，不能选择明细包装」；已填包装但子表为空用「包装「x」未维护明细包装，请到基础数据-包装代码里添加」，不再误报未填写包装。师傅 `userAttribute=512`，人数前后端都不卡。按状态：**工单主表**未提交（保存/删除/提交）、待认领（仅撤回）、已认领与已完成（主表全部禁用并提示联系监装师傅）；**箱型附件分组**不受工单状态限制。工单号由后端按 `LoadingOrder.LoadingOrderNum` 生成。干系人里的「监装」与工单师傅列表是两套，互不同步。
...
| **码头航次** | 港区航次，与船公司航次 `innerVoyno` 是两套编号；界面不展示。 | `SeaExportDto.terminalVoyno` | **触发/依赖：** 表单 `hidden`；码头船舶引入写这里（`evoyage`），不写 `innerVoyno`。 | 可空；上限 64。 |
| **起运预计离开（运踪）** | 非 sjtd 运踪摘要：接货地预计离开。 | 详情 `feituoTracking.polEtd` ← 库 `ReceiptEtd` ← 飞驼 `result.receipt.etd` | **触发/依赖：** `SeaExportAdmin` 详情；`FeituoTrackingQueryManager` 把 `ReceiptEtd` 映成 `PolEtd`。勿与基础信息 `transportOrder.etd` 混用。 | 只读字符串；空显示 `--`。 |
...
>
> **[卡点 13：分单头备注与装箱行备注不是同一字段]** 代理地址右侧「备注」写 `SeaExportSeparate.Remark`；装箱表「备注」列写 `SeaExportSeparateCtn.Remark`。复制分单只复制主表备注，不带装箱子表。
>
> **[卡点 14：分单件数大写只在页面拼]** SAY 行由前端按件数+包装名计算，不读主单 `transportOrder.upperPKGS`。分单打印走数据源 500，后端 `SeaExportSeparateDto` 也尚未加 SAY 字段，模板若要件数大写需自行按件数/包装拼。
>
> **[卡点 15：监装照片不是工单编辑]** 管理端改某箱监装照片走 `EditOrderCtnAttachmentGroupsAsync`，`id` 是箱型 id 不是工单 id；不要塞箱号/封号/是否完成，也不要再调师傅端 `EditOrderCtnsAsync`（那条仍要求已认领且会改箱子字段）。弹窗先按 `160100` 铺维护类型，再叠已有照片；附件按该箱全量替换，漏传某组等于删掉该组照片。每分类可多选追加，批量上传失败保留成功图片；上传中不可关闭或保存。仅新上传图片写入水印，历史图片保持原样；PC 水印时间为设备时间转换后的北京时间，不包含位置，也不申请定位权限。
>
...
| 2026-09-09 | `Fix` | 分单：备注挪到代理地址旁；货物区底部前端拼件数大写 `SAY:… ONLY.`；装箱改件数同步刷新。复制能力沿用既有 `CopyAsync`。 | TAPD `#1161580498001000934`。不套主单 `upperPKGS`。详见 `changelogs/change-log-2026-09-09-sea-export-separate-agent-remark-pkgs-say.md`。 |
...
| 2026-09-05 | `Feature` | 监装「照片采集」先拉维护的附件类型再按类型分槽上传，与小程序一致。 | `GetListByModuleTypesAsync(160100)`；空槽 + 已有照片合并。详见 `changelogs/change-log-2026-09-05-admin-loading-photo-types.md`。 |
| 2026-09-05 | `Feature` | 监装 Tab 增加「分享」：复制免登录链接给客户查看监装信息，公开页不展示监装要求与备注。 | `LoadingOrder/DetailByMblAndLoadingOrderNumAsync` + `skipAuth`；链接 `router.resolve`。详见 `changelogs/change-log-2026-09-05-loading-order-share.md`。 |
...
| 2026-09-01 | `Parsing` | 无 | 运踪「起运预计离开 / 目的预计到达」取详情 `feituoTracking.polEtd` / `podEta`（空则 `podSta`），对应库 `ReceiptEtd` / `DeliveryEta`，不是业务单 `etd/eta`。详见 `parsing-logs/parse-log-2026-09-01-sea-export-tracking-pol-etd-pod-eta.md`。 |
...
| 2026-09-13 | `Fix` | 监装 Tab 去掉单独的「查看监装视频」按钮，看直播只走分享预览。 | 避免与客户页入口重复占位。 |
| 2026-09-13 | `Feature` | 监装 Tab 新增摄像头只读字段与视频查看入口。 | 摄像头与直播按监装视频接口文档对接，所有应用服务方法保留 Async 后缀。 |
| 2026-09-15 | `Feature` | 字段权限展示：无条件受限列与筛选隐藏；条件受限单元格显示 `***`；编辑表单按原始 DTO 缺 key 隐藏项目，费用受限格禁止编辑。 | 显式模块配置、原始键结构快照及共享展示适配器。 |

| 2026-09-18 | `Feature` | 监装每分类支持多图，批量上传，新增上传人与北京时间水印。 | 串行队列完成前禁止保存；附件分组仍全量替换，水印固化为 JPEG。详见 [变更记录](../../changelogs/change-log-2026-09-18-监装分类多图与上传水印.md)。 | | 2026-09-18 | `Feature` | 监装照片新增上传位置水印，定位成功后才允许上传。 | 每批共用一次浏览器定位，禁用旧位置缓存，失败整批阻止上传。详见 [变更记录](../../changelogs/change-log-2026-09-18-监装水印增加位置并强制定位.md)。 | | 2026-09-19 | `Feature` | PC 取消位置水印与强制定位，仅保留上传人和时间。 | 小程序继续强制定位；共享布局以显式 null 区分 PC，空值不会被当成小程序定位成功。 |

| 2026-09-19 | `Fix` | 新上传监装照片水印上下留白对称，移除末行多余行距，文字按行中心绘制。 | PC 与小程序共用布局，历史图片不变。 |

| 2026-09-23 | `Feature` | 空箱箱内置顶独占整行并支持多图，其他分类并排且限单张，保留历史附件。 | 上传入口及串行队列限制追加；详见 [变更记录](../../changelogs/change-log-2026-09-23-监装空箱箱内多图与附件排列.md)。 |
