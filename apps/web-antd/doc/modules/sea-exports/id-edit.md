...
>
> **[卡点 14：分单件数大写只在页面拼，打印数据源还没有]** SAY 行由前端按件数+包装名计算，不读主单 `transportOrder.upperPKGS`。分单打印仍是海出详情数据源；后端 `SeaExportSeparateDto` 尚未加对应字段，模板绑不到这一项。
>
...
| 2026-09-09 | `Fix` | 分单：备注挪到代理地址旁；货物区底部前端拼件数大写 `SAY:… ONLY.`；装箱改件数同步刷新。复制能力沿用既有 `CopyAsync`。 | TAPD `#1161580498001000934`。不套主单 `upperPKGS`；打印数据源未加字段。详见 `changelogs/change-log-2026-09-09-sea-export-separate-agent-remark-pkgs-say.md`。 |
