...
- **工作台 Tab 记忆：** 切换顶部标签时，按当前委托 ID 将 `activeTab` 写入 `sessionStorage`（键经 `buildBrandStorageKey` 品牌隔离）；再次进入同一票编辑页时自动恢复离开前的 Tab。仅恢复当前可见且有对应面板的 Tab key；关闭浏览器标签后会话清空，下次默认回到「基础信息」。路由查询参数 `tab`（如工作台完成缺附件跳转的 `?tab=attachments`）优先于会话记忆，命中后会 `replace` 掉该参数以免挡住下次深链。基础信息表单内滚动**不再**改写工作台 `activeTab`（已移除分区 Tab 双向联动）。
...
| 2026-08-19 | `Fix` | 工作台完成缺附件会深链进本页附件 Tab（`?tab=attachments` 覆盖会话记忆）。 | `editor.vue` 优先读 `route.query.tab`，命中后 `replace` 去掉该参数。详见 `changelogs/change-log-2026-08-19-se-service-require-attachment-types.md`。 |
| 2026-08-19 | `Feature` | 完成前需上传提示可点击，跳转附件 Tab 上传；缺附件预检失败也会自动切过去。 | `form.vue` emit `switch-tab`，`editor.vue` 写入 `activeTab`。详见 `changelogs/change-log-2026-08-19-se-service-require-attachment-types.md`。 |
