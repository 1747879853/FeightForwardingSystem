...
| :-- | :-- | :-- | :-- |
| 2026-07-11 | `Feature` | 顶栏订阅按钮旁展示运踪订阅状态 Tag；按钮文案随状态在「运踪订阅/重新订阅」切换，`订阅成功`时禁用并提示「已成功订阅」；订阅后重拉详情刷新状态。 | `loadEditData` 回填 `isYundangSubscribed`/`isYundangSubscribeSuccess`；复用 `getYundangSubscribeStatus(Meta)`；`success` 禁批量订阅，`failed` 复用批量接口重试。 |
