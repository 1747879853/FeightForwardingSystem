...
| **监装箱型附件分组** | 某只箱子的监装照片，按附件明细类型分组。 | 类型 `AttachmentDtlType/GetListByModuleTypesAsync`（`160100`）；已有照片 `orderCtns[].attachmentGroups`；保存 `LoadingOrderAdmin/EditOrderCtnAttachmentGroupsAsync` | **触发/依赖：** 弹窗先铺维护类型空槽再填已有照片；类型横排网格；上传走通用 `UploadFile` 拿 `attachmentId`。`id` 必须是 `OrderCtn.Id`。 | 需 `.Edit`；任意工单状态可调；**每类型最多 1 张**；全量替换，漏传某组等于删除，`[]` 清空。 |
...
| :-- | :-- | :-- | :-- | --- | --- | --- | --- |
| 2026-09-15 | `Feature` | 费用录入表格接入字段权限：条件屏蔽显示 `***`，非条件屏蔽隐藏列。 | 共享 Handsontable；规则在 `_shared/order-fee/field-permission.ts`。详见 [变更日志](../../changelogs/change-log-2026-09-15-sea-export-order-fee-field-permission.md)。 |
| 2026-09-15 | `Feature` | 基础信息接入字段权限：条件/非条件屏蔽都显示 `***`，不隐藏表单项。 | `applyFormFieldMasks`；页头委托编号/组织/装运方式同步打码；保存剔除 `***`。详见 [变更日志](../../changelogs/change-log-2026-09-15-sea-export-field-permission.md)。 |
