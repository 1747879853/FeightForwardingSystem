...
> [!IMPORTANT] **[卡点 9：分类多图与上传水印]** 每分类支持持续追加，拍照一次一张、相册一次最多九张。新图片上传前写入登录人和设备时间（北京时间）水印，历史图片保持原样。失败保留已成功图片，需重新选择失败图片；上传或保存期间禁止关闭与删除，上传完成前不能保存。水印生成失败不回退原图。
...
| 2026-09-18 | `Feature` | 每分类支持多图与持续追加，相册一次九张；新图片添加上传人与时间水印。 | 画布串行绘制并导出 JPEG 后上传；失败保留成功项、禁止上传中保存关闭。详见 [变更记录](../../changelogs/change-log-2026-09-18-监装分类多图与上传水印.md)。 |
