...
- **底栏四个 Tab：** 首页、内部首页、积分兑换、个人中心。第一期只有「内部首页」（监装列表）与「个人中心」有内容，首页与积分兑换是占位页。
- **列表（`pages/loading/list`）：** 顶部分段「新派 / 进行中 / 已完成」分别打接口状态 1/2/3；卡片展示监装工号、状态徽标、主提单号、明细包装\*件数、船名航次、堆场、品名、下单日期与预计到货日期。支持下拉刷新、触底加载。
- **检索：** 点顶栏放大镜展开，支持监装工单号（模糊）、主提单号（模糊）、预计到货日；有生效条件时放大镜带红点。
- **详情（`pages/loading/detail`）：** 三张卡——基本信息（13 行）、监装要求（标签 + 详细说明）、集装箱要求（序号/箱型/箱号/封号/监装图片/监装状态）。
- **按状态的底部操作条：**
  - 待认领 → 「认领」
  - 已认领 → 「拒接」（必填原因）、「保存」
  - 已完成 → 「取消完成」
- **箱型编辑：** 只有已认领状态可改箱号、封号、完成勾选；不能加箱删箱。
- **监装照片：** 点行内「监装图片」弹出该箱面板，按附件明细类型分组展示；可拍照或从相册选，上传后先记在本地，点页底「保存」才随箱一起提交。
...
> [!IMPORTANT] **[卡点 1：入口靠用户属性，不是权限码]** 师傅端六个接口都不校验后台权限，只查当前登录人的 `UserAttribute` 是否含监装（512）。缺属性时后端报「当前登录人的用户属性不包含监装,无权使用监装功能」，前端捕获该文案后把内部首页整页降级成「当前账号无监装权限」，其余 Tab 照常可用。 `GetMyAsync` 目前不返回 `userAttribute`，所以只能靠接口报错判断，无法提前预判。
