...
- **监装处理：** 箱行只留一个入口，展示待处理/已完成与已传张数；点开面板可改该箱箱号、封号、完成状态并按附件类型分区传图（空箱箱内置顶独占整行并支持多张、缩略图每行三张；其他分类每类限一张，三列并排，标题显示张数）。箱号旁可「识别」：拍照/相册一张图只用来识别箱号，不进入监装照片。已认领时面板底栏点「保存」立即提交；点遮罩或关闭且未保存则还原该箱打开时的值。拍照与相册分入口，避免误开相机。
- **箱型编辑：** 只有已认领状态可在「监装处理」面板改箱号、封号、完成勾选；列表行只展示。不能加箱删箱。
- **按状态的底部操作条：**
  - 待认领 → 「认领」
  - 已认领 → 「拒接」（必填原因）；箱号/封号/照片/完成状态在监装处理面板内保存
  - 已完成 → 「取消完成」
- **监装照片：** 详情加载时并行调 `AttachmentDtlType/GetListByModuleTypesAsync`（`moduleTypes: [160100]`），把维护的类型铺成空槽，再叠上该箱已有 `attachmentGroups`，空箱箱内置顶独占整行，其余按类型原始 `sortId` 降序横排。空箱箱内支持多张，相册一次最多九张；其他类型限 1 张，有图则隐藏添加槽，相册/拍照都只选 1 张。拍照先出本地缩略图再上传，展示地址必须拼 `VITE_API_ORIGIN`。上传后先记本地，点「保存」才随箱提交。未配置类型时提示「未配置监装附件类型」，并保留一个未分类「监装照片」槽。
...
| **attachmentGroups** | 分组监装照片 | 详情 `orderCtns[].attachmentGroups` + `AttachmentDtlType/GetListByModuleTypesAsync`（`160100`） | **触发/依赖：** 先铺维护类型空槽再填已有照片；类型横排；**按箱全量替换**，漏传该组等于清空该组 | 先 `POST /upload/UploadFile` 拿 `attachmentId`；**空箱箱内支持多张，其他类型最多 1 张；历史多图保留**；空组不提交 |
...
| **submitUserName / submitUserPhone** | 派单人姓名 / 手机号 | `GetMyPagedListAsync` | 有姓名才在标题行徽标左侧显示灰色小字；点姓名弹窗确认后 `makePhoneCall`。无手机号 toast「该派单人未维护手机号」 | 姓名空则不显示 |
...
> [!IMPORTANT] **[卡点 9：分类多图与上传水印]** 仅空箱箱内支持持续追加，拍照一次一张、相册一次最多九张；其他分类限一张，已有图后隐藏添加入口，历史多图保留。进入详情时获取一次 GCJ-02 位置并通过腾讯逆地址解析，同任务各箱及连续上传复用该地址；应用进入后台时清空旧地址，返回详情后重新获取；新图片写入登录人、设备时间（北京时间）及中文地址水印；定位失败、拒绝授权、超时或地址解析失败均阻止上传，可点击“重新定位”；照片面板不再常驻多图与水印长说明，仅保留定位中、位置或简短错误；详情和照片面板采用左侧提示、右侧小号操作“重新定位”和“开启定位”的同行布局，长提示允许换行，允许位置权限后返回自动重新定位并解析地址，仍未授权继续阻止上传；设置打开失败提示右上角“…”→设置→位置信息的手动路径。历史图片保持原样。失败保留已成功图片，需重新选择失败图片；上传或保存期间禁止关闭与删除，上传完成前不能保存。水印生成失败不回退原图。部分安卓选图使用原图，避免系统压缩直接交出黑图或红图；水印改由隐藏的 `lime-painter@11.11.11` 画板串行绘制照片、上传人、北京时间和中文地址，再导出 JPG；先调用 render 重置完成状态，再登记导出任务，避免复用上一张图片。导出超过 15 秒提示重试。画板由组件管理，不再手工读取像素或重画。
...
| 2026-09-13 | `Fix` | 修复 token 失效后登录页与工单列表循环跳转、重复报未登录。 | 共享登录状态，401 同步清缓存与内存；个人资料鉴权失败不视为登录成功。 |
| 2026-09-14 | `Fix` | 箱列表进一步压缩为两行主体布局，整行点击进入面板，移除独立操作按钮。 | 最小高 76px；长编号和照片数允许自然撑高。 |
| 2026-09-14 | `Fix` | 列表派单人从标题行拆成独立拨打条，避免和进详情抢点击。 | 原生 button + tap.stop；姓名空不渲染。`GetMyPagedListAsync` 已出 `submitUserName` / `submitUserPhone`。 |
| 2026-09-14 | `Fix` | 派单人改成与船名/堆场同结构的信息行，电话明文，只有「拨打」可点。 | 去掉整宽蓝底操作条；点姓名仍进详情。 |
| 2026-09-14 | `Fix` | 派单人改回标题行徽标左侧灰色小字。 | 20rpx `$text-label`；点姓名 `tap.stop` 后弹窗拨打。 |
| 2026-09-14 | `Fix` | 修复点卡片其它区域也会弹出拨打。 | 整卡不再绑 tap；进详情绑工号/徽标/主内容；拨打只用原生 button + hover-stop-propagation。 |

| 2026-09-18 | `Feature` | 每分类支持多图与持续追加，相册一次九张；新图片添加上传人与时间水印。 | 画布串行绘制并导出 JPEG 后上传；失败保留成功项、禁止上传中保存关闭。详见 [变更记录](../../changelogs/change-log-2026-09-18-监装分类多图与上传水印.md)。 | | 2026-09-18 | `Feature` | 监装照片新增上传位置水印，必须定位成功才能上传。 | 每批重新取 GCJ-02 经纬度；微信定位权限声明已补齐，失败无降级上传。详见 [变更记录](../../changelogs/change-log-2026-09-18-监装水印增加位置并强制定位.md)。 | | 2026-09-19 | `Feature` | PC 取消位置水印，小程序继续要求定位成功后上传。 | 小程序仍传入有效 GCJ-02 坐标；中文地址逆解析尚未接入。 | | 2026-09-19 | `Feature` | 监装水印中文地址接入腾讯逆地址解析，长地址自动换行。 | 复用现有地图 Key，每批 GCJ-02 定位后解析一次完整地址，解析失败中止上传；PC 不获取位置。详见 [变更记录](../../changelogs/change-log-2026-09-19-小程序监装水印接入中文地址.md)。 | | 2026-09-19 | `Feature` | 进入详情获取一次任务地址，连续上传跨箱复用；后台返回刷新位置。 | 应用级后台版本号使缓存失效，普通相机/相册页面返回不重复定位；过期请求不能覆盖新结果。详见 [变更记录](../../changelogs/change-log-2026-09-19-监装详情复用位置与后台返回刷新.md)。 |

| 2026-09-19 | `Fix` | 定位拒绝后可从详情或照片面板打开设置重新授权，返回后自动重新定位。 | 检查位置授权结果并使旧请求失效，未授权继续阻止上传；详见 [变更记录](../../changelogs/change-log-2026-09-19-监装定位拒绝后手动开启授权.md)。 |

| 2026-09-19 | `Style` | 定位重试与权限设置改为提示下方并排小号文字操作，移除整行按钮边框背景。 | 保留原生按钮点击语义，清除 ::after 描边，详情和照片面板统一。 |

| 2026-09-19 | `Style` | 删除照片面板常驻上传长说明，仅保留必要定位状态、简短失败原因和操作入口。 | 上传限制及授权恢复逻辑不变。 |

| 2026-09-19 | `Style` | 定位提示与操作同行，操作精简为重新定位、开启定位。 | 左侧允许换行，右侧操作不收缩。 |

| 2026-09-19 | `Fix` | 新上传监装照片水印上下留白对称，移除末行多余行距，文字按行中心绘制。 | PC 与小程序共用布局，历史图片不变。 |

| 2026-09-23 | `Feature` | 空箱箱内置顶独占整行并支持多图，其他分类并排且限单张，保留历史附件。 | 上传入口及串行队列限制追加；详见 [变更记录](../../changelogs/change-log-2026-09-23-监装空箱箱内多图与附件排列.md)。 | | 2026-09-25 | `Fix` | 部分安卓监装照片不再导出黑底或红底纯色图。 | 选原图，页面内 2d 画布确认白字后再导出；详见 [变更记录](../../changelogs/change-log-2026-09-25-小程序监装水印避免安卓纯色图.md)。 |

| 2026-09-25 | `Fix` | 监装照片水印改用隐藏画板生成 | 接入 lime-painter，修正测试类型并补齐小程序编译器依赖；14 项水印单测和微信构建通过，真机效果待验证 |
