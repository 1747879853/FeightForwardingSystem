...
**白话解释：** 维护业务编号生成规则，支持组织、用户或全局范围的编号策略。
...
- **列表维护：** 在 `编号规则` 页面查询、创建、编辑和删除基础资料。
- **弹窗表单：** 多数基础资料通过 `GenerateNumAdmin/modules/form.vue` 维护明细。
...
> [!IMPORTANT] **[卡点 1：编号规则一致性]** 基础资料页面同质性高，但字段变更会影响大量业务下拉，需谨慎维护编码和启用状态。
