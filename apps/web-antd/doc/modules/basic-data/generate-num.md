...
| :-- | :-- | :-- | :-- | :-- |
| **表名** | 编号规则绑定的业务实体字段。 | `GenerateNumAdmin/data.ts` 固定下拉 | 与后端 `Entity.Field` 一致，含 `SeaExport/SeaImport/AirExport.CommissionNum`、`InvoiceApplication.ApplicationNo`、`InvoiceIssue.ApplicationNo`、`PreOrder.PreOrderNum` 等。 | 必选，预置 11 项。 |
...
> [!IMPORTANT] **[卡点 3：业务日期规则的取值来源]** `业务日期(yyyyMM)/(yyMM)` 取本票 ETD，由后端 `GetGenerateTextDto.ETD` 传入；ETD 为空时回退系统当前时间。各业务 ETD 含义：海运出口=开船日期、海运进口=到港日期、空运出口=起飞日期。配置页预览无单据上下文，一律按当前时间展示，与实际生成结果可能不同。
