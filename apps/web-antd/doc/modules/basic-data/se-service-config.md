...
- **主流程标记：** 每条服务项明细标题栏在「排序」后可维护「是否主流程（全局）」；该开关写入 `ServiceType` 枚举子项 `extra1`，同一服务项目在所有港口共用，不是港口级字段。
...
| **extra1 / 是否主流程** | 服务项目是否属于海运出口业务主流程。 | `ServiceType` 枚举子项 `extra1` | 港口配置弹窗可直接修改；海运出口配置服务项目弹窗据此分成「主流程 / 非主流程」。 | 全局标记，同一 `serviceType` 在所有港口一致；修改需要枚举详情与编辑权限。 |
