...
**白话解释：** 打开一票已存在的空运出口委托，在基础信息、费用总览、附件三个标签之间切换维护。
...
| 权限口径 | `Admin.AirExport`（编辑 `Admin.AirExport.Edit`） |
| 关键源码 | `src/views/air-export-admin/editor.vue`<br/>`src/views/air-export-admin/basic-info-form/form.vue`<br/>`src/views/air-export-admin/orderFee/index.vue`<br/>`src/views/air-export-admin/attachments/index.vue` |

# 2. 功能与操作说明 (Features & Operations)

- **标签容器：** 基础信息 / 应收应付（只读）/ 附件三个标签，按委托 ID 记忆上次停留的标签。
