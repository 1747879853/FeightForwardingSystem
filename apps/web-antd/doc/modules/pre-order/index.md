...
- **进入单据：** 双击行按状态分流——「录入 / 驳回」→ `/pre-order/:id/edit`；「待审核 / 通过」→ `/pre-order/:id/detail`（`getPreOrderFormPath`）。
...
| :-- | :-- | :-- | :-- |
| 2026-07-26 | `Feature` | 双击行按状态进编辑或详情：待审核/通过打开 `/detail`，录入/驳回仍进 `/edit` | 分流逻辑抽到 `getPreOrderFormPath`，列表不再写死 `/edit` |
