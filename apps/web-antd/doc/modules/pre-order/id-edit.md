...
- **费用工具栏：** 可编辑时提供单一「添加」与「删除」icon 按钮（样式对齐海运出口箱型箱量）；新增行默认应收 + USD + 汇率，收付在行内切换；费用代码带出类别/结算/币别/税率/单位；单位切换按票·重量·体积·TEU·箱型自动带量（数量只读）；须先勾选行再删。
...
| **费用.收付类型** | 应收(0) / 应付(1) | 固定选项 | **触发：** 切到应收且单位是箱型时立即带出卖价与箱量；切换后按收付重取汇率；若已有费用代码则按应收/应付重带行业类别与结算对象 | —— |
| **费用.费用代码** | 费用名称来源 | **基础数据**<br/>`FeeCodeSelect` | **触发：** 带出行业类别（应收 `defaultDebitName` / 应付 `defaultCreditName`）、结算对象、币别+汇率、税率、默认单位、禁开票/机密 | —— |
...
| **费用.单位** | 计价单位 | 通用单位（票 / 重量 / 体积 / TEU，取自 `PRE_ORDER_GENERIC_UNITS`）+ 已选箱型名 | **默认：** 已有箱型时取首个箱型名；费用代码默认单位经 `normalizePreOrderFeeUnit` 归一后写入，不可识别时退回「票」并提示；**触发：** 按单位自动带出数量（见「费用.数量」） | 最长 16，必须是后端 `ResolveQuantityByUnit` 可识别的字面量，提交审核前会校验 |
| **费用.含税单价** | 对客单价 | 手填或箱型卖价 | **依赖：** 应收 + 单位命中箱型时只读，取箱型卖价；应付全程手填 | 非负 |
| **费用.数量** | 计价数量 | 完全由单位派生，**始终只读**（后端审核通过时按单位重算并覆盖，手改无意义） | **触发：** 票=1；重量=货物 `kgs`；体积=`cbm`；TEU=各箱型 `teu×箱量` 累加；箱型名=箱量；单位不可识别=0。箱型卖价/箱量、毛重/体积变更后全表重新带量 | 非负 |
| **费用.税率(%)** | 税率 | 手填或费用代码带出 | **触发：** 重算不含税单价 | —— |
| **费用.不含税单价** | 去税单价 | 计算列 | `unitPrice / (1 + taxRate/100)`，只读 | —— |
| **费用.金额** | 含税金额 | 计算列 | **触发：** 新增行即按含税单价 × 数量重算；之后单价/数量/税率变更、单位/收付切换、箱型卖价箱量同步时同步重算 | 始终只读 |
...
> [!IMPORTANT] **[卡点 6：费用单位字面量是与后端的硬契约]** 审核通过生成 `OrderFee` 时后端 `ResolveQuantityByUnit` **精确匹配**「票 / 重量 / 体积 / TEU + 箱型名」且不做别名映射 -> 单位下拉只能给这些值；写成海出口径的「毛重 / 尺码 / 件数」会落入 default 分支按箱型名匹配失败，数量与金额落库为 `0`。业务联系单**不支持按件数计价**。
...
> [!IMPORTANT] **[卡点 8：数量以单位重算结果为准]** 联系单录入的 `quantity` 在转换时被完全丢弃 -> 数量列只读、恒由单位派生；箱型卖价/箱量、货物毛重/体积变更后全表重新带量，保证「预估看到的」= 「落库算出的」。
