...
| **任务类型** | 新增、修改、删除等费用动作。 | 费用任务详情 / `_shared/order-fee` | **触发/依赖：** 决定详情页展示和审核含义。 | 需与费用提交 DTO 对齐。 |
...
| 2026-07-12 | `Fix` | 费用审核列表仅点击 radio 才选中，单击行不再切换选中。 | `radioConfig.trigger` 由 `'row'` 改为 `'default'`（`expense-all`）。 |
