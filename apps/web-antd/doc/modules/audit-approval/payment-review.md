...
**白话解释：** 财务或审批人集中处理付款申请审批任务。选中列表中的申请后，可在同页查看结算对象应收未结算、发票、附件与费用明细，再执行「通过」或「驳回」。菜单文案现为「付费申请审批」。
...
- **选中查看详情：** 点击列表行，右侧依次展示结算对象应收未结算、独立发票表和附件区，下方通铺费用明细分组表（只读，业务组默认收起，需手动展开看费用行；表格占满剩余高度内部滚动，最后一行费用可完整看到），发票表展示抬头/票号/日期/金额/附件及总额，附件区展示申请自身分组附件、结算附件；点击附件打开全站附件查看器（图片/PDF 弹窗预览，Office 用 vue-office 本地渲染）。
