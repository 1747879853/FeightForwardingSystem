...
- **上一票 / 下一票：** 工作台顶栏右侧。详情带上最近一次海进列表的筛选和排序，按全量顺序取 `previousId` / `nextId`。Id 为空则禁用；未保存离开走现有脏检查。
...
> [!IMPORTANT] **[卡点 4：飞驼航次是码头航次]** 码头船舶条目的 `ivoyage` 必须写入 `terminalVoyno`，禁止回填 `innerVoyno`。进口按起运港查，国外港经常查不到，与本次字段无关。
