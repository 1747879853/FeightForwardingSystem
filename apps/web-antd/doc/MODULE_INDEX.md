...
| fee-management | `/fee-management/payment-application/add` | 费用管理 | 创建付款申请；发票子表含销售方抬头与金额，总额前端求和；可从进项发票勾选回填（已填票号排除，已选银行带开票信息 id）；发票行上传即自动识别回填，申请附件分组上传不识别；先票后付可先空着保存、提交时才必填；可从海出/海进/空出应收应付带 orderFeeIds 预填（同一套发票校验）；指定结算币别折算预填只取汇率表原币兑结算币；添加费用抽屉可按对账单号模糊检索，可筛业务类型，按业务简要读港口备注，按币别展示已选合计并保留跨页勾选。 | [付款申请新增](./modules/fee-management/payment-application-add.md) | 2026-09-08 |
| fee-management | `/fee-management/payment-application/:id/edit` | 费用管理 | 编辑付款申请；发票子表含销售方抬头与金额，总额前端求和，保存全量覆盖；可从进项发票勾选回填；发票行上传即自动识别回填，申请附件分组上传不识别；先票后付提交时才要求至少一条发票；指定结算币别折算预填只取汇率表原币兑结算币；添加费用抽屉可按对账单号模糊检索并筛业务类型，按业务简要读港口备注；驳回后可再次提交。 | [付款申请编辑](./modules/fee-management/payment-application-id-edit.md) | 2026-09-08 |
