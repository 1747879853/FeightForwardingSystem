...
| pre-order | `/pre-order/add`、`/pre-order/:id/edit` | 业务联系单 | 业务联系单工作台：主表按设计稿排位（起运/目的港进基础字段、收发通内嵌折叠）；港口 5 列分区 `hidden` 暂隐，主表选港同步 `PortForm` 并带备注；货物区内容高度自适应；费用表仅展示计价列；附件分组随 Add/Edit 全量提交；可一键生成应收海运费；录入/驳回可保存提交，待审核/通过仅审核；通过后内嵌海出；干系人角色按业务类型枚举；中转港 `poT1Id`/`poT2Id`；委托单位 `industryCategory=p`。 | [业务联系单编辑](./modules/pre-order/id-edit.md) | 2026-08-12 |
