...
| pre-order | `/pre-order/add`、`/pre-order/:id/edit` | 业务联系单 | 业务联系单工作台：主表含订舱代理（`bookingAgentId`/`industryCategory=o`）与起运/目的港，收发通内嵌折叠；港口分区 `hidden` 暂隐；费用结算对象可带出订舱代理；附件随 Add/Edit 全量提交；可一键生成应收海运费；录入/驳回可保存提交，待审核/通过仅审核；通过后内嵌海出并将订舱代理写入海出。 | [业务联系单编辑](./modules/pre-order/id-edit.md) | 2026-08-13 |
