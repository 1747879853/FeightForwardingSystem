...
> **2026-09-07 更新：**
>
> 1. 新增 `UploadAndExtractInvoiceAsync`：上传一个发票文件(PDF或图片)，**一次请求完成落附件 + 识别发票**，出参在通用上传结果上增加嵌套的 `invoice` 对象，识别失败为 `null`。原 `ExtractInvoiceAsync`（只识别不落库）**保持不变**。付费申请发票行上传走此接口，不必再手动点识别。详见「8」。
...
付费申请**发票行**优先走本接口：上传即回填当前行，不必再点识别。申请附件分组（含名称含「发票」的分组）走通用上传，不走本接口。已有附件要重新识别时仍用 `ExtractInvoiceAsync`。
