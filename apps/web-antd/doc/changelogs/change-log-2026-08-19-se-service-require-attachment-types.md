...
3. **完成任务**
   - 海出编辑页 Tooltip 提示「完成前需上传」；点击提示或「去上传」切到附件 Tab。点完成前按当前任务 + 已完成任务累积配置预检本票附件，缺则提示并自动跳转附件页。
   - 工作台点完成会先按当前服务项配置预检本票附件：缺则提示并跳到对应海出编辑页附件 Tab（`?tab=attachments`，批量时打开第一条缺附件的票）；已传齐才弹出确认框。确认框里的「完成前需上传」可点「去上传」。后端仍报缺附件时也会跳转。
...
- 工作台跳附件必须带 `query.tab=attachments`，且 `editor.vue` 要让该参数覆盖 sessionStorage 的 Tab 记忆，否则会回到上次停留的「基础信息」。进页后会 `replace` 掉 `tab` 参数，方便同一票再次深链。
