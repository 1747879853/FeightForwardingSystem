...
export function isExchangeRateEffectiveOn(
  ...
  const day = dateStr ? toDateKey(dateStr) : undefined;
...
export function peekExchangeRateOnDate(
  ...
  // 本位币匹配的记录优先（严格模式只认这类记录）
  if (localKey !== '') {
    for (const item of list) {
      if (String(item.localCurrencyId ?? '') !== localKey) continue;
      if (!isExchangeRateEffectiveOn(item, dateStr)) continue;
      if (!rate || isPreferred(item, rate)) rate = item;
    }
  }
  // 非严格模式：没有本位币匹配记录时，兜底取任意有效记录（兼容旧调用方）
  if (!rate && !strict) {
    for (const item of list) {
      if (!isExchangeRateEffectiveOn(item, dateStr)) continue;
      if (!rate || isPreferred(item, rate)) rate = item;
...
/**
 * 读取已加载缓存中的生效汇率（按今天判有效期），语义同
 * {@link peekExchangeRateOnDate} 的非严格模式，兼容既有调用方。
 */
