...
const Submitted = (approve: boolean, modalRemark: string) => {
  if (!selectedRowKeys.value.length) return;
  const keysSet = new Set(selectedRowKeys.value);
  const list = (dataSource.value ?? []).filter((row) =>
    keysSet.has((row as any)._rowKey),
...
const columns = [
  ...
  },
  {
    ...
    children: [
      {
        title: orderFeeDataT('invoiceStatus'),
      ...
      {
        title: clientDataT('industryCategories'),
      ...
      {
        title: orderFeeDataT('rqstPaymentAmount'),
        dataIndex: 'rqstPaymentAmount',
        key: 'rqstPaymentAmount',
        minWidth: 80,
      },
      {
        title: orderFeeDataT('invoicedAmount'),
        dataIndex: 'invoicedAmount',
        key: 'invoicedAmount',
        minWidth: 80,
      },
      {
        title: orderFeeDataT('orderInvoiceAmount'),
        dataIndex: 'orderInvoiceAmount',
        key: 'orderInvoiceAmount',
        minWidth: 80,
      },
      {
        title: orderFeeDataT('settledAmount'),
        dataIndex: 'settledAmount',
        key: 'settledAmount',
        minWidth: 80,
      },
      {
        title: orderFeeDataT('canInvoice'),
        dataIndex: 'invoiceBlocked',
        key: 'invoiceBlocked',
        minWidth: 80,
      },
      {
        title: orderFeeDataT('isConfidential'),
        dataIndex: 'isConfidential',
        key: 'isConfidential',
        minWidth: 80,
      },
      {
        title: orderFeeDataT('dataEntryMethod'),
