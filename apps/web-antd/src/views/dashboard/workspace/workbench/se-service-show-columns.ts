...
const PROP_VALUE_RESOLVERS: Record<number, PropValueResolver> = {
  ...
  1002: (seaExport) =>
    formatText(seaExport?.pol?.portName || seaExport?.pol?.cnName),
  1003: (seaExport) =>
    formatText(seaExport?.pod?.portName || seaExport?.pod?.cnName),
  ...
  1013: (seaExport) => formatText(seaExport?.codeIssueType?.billType),
