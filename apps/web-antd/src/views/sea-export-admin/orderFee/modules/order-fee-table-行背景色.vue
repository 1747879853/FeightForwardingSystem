...
// 根据费用状态获取行背景色
const getRowBackgroundColor = (feeStatus: number) => {
  const statusOption = getFeeStatusOptions().find(
    (item) => item.value === feeStatus,
  );
...
const [Grid, gridApi] = useVbenVxeGrid<OrderFeeAdminApi.OrderFeeDto>({
  gridOptions: {
    ...
    // 添加行样式函数，根据费用状态设置行背景色
    rowClassName: ({ row }) => {
      const feeStatus = row.feeStatus;
      if (feeStatus !== undefined && feeStatus !== null) {
        ...
        if (statusOption) {
          // 返回一个带有背景色的CSS类名
          return `fee-status-row-${feeStatus}`;
        }
      }
      return '';
    },
...
<style scoped>
/* 根据费用状态为行添加背景色 */
:deep(.fee-status-row-0) {
  background-color: #b8cdd7 !important;
}

:deep(.fee-status-row-1) {
  background-color: #ffc107 !important;
}

:deep(.fee-status-row-2) {
  background-color: #67c23a !important;
}

:deep(.fee-status-row-3) {
  background-color: #f56c6c !important;
}

:deep(.fee-status-row-4) {
  background-color: #f90 !important;
}

:deep(.fee-status-row-5) {
  background-color: #f90 !important;
}

:deep(.fee-status-row-6) {
  background-color: #909399 !important;
}

:deep(.fee-status-row-7) {
  background-color: #67c23a !important;
}
</style>
