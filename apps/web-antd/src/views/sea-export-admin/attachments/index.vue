...
interface AttachmentTypeGroup {
  ...
  name: string;
  sortId: number;
  items: SeaExportAdminApi.AttachmentItemDto[];
}
...
const allAttachmentTypes = ref<SeaExportAdminApi.AttachmentDtlTypeSimpleDto[]>(
  [],
);
/** 用户手动添加的非默认展示类型 */
...
const resolveGroupName = (
  typeId: number | null,
  typeInfo?: SeaExportAdminApi.AttachmentDtlTypeSimpleDto | null,
) => {
  if (typeInfo?.name) return typeInfo.name;
  if (typeId === null) {
    return $t('seaExport.export.attachments.uncategorized');
  }
  return String(typeId);
};

const mergeGroups = (
  configuredTypes: SeaExportAdminApi.AttachmentDtlTypeSimpleDto[],
  attachmentGroups: SeaExportAdminApi.AttachmentGroupDto[],
  extraTypeIds: number[] = [],
  allTypes: SeaExportAdminApi.AttachmentDtlTypeSimpleDto[] = [],
): AttachmentTypeGroup[] => {
  const itemsByTypeId = new Map<
    number | null,
    SeaExportAdminApi.AttachmentItemDto[]
  >();
  const typeInfoById = new Map<
    number | null,
    SeaExportAdminApi.AttachmentDtlTypeSimpleDto | null
  >();
...
const confirmAddOtherType = () => {
  if (selectedOtherTypeId.value == null) {
    message.warning($t('seaExport.export.attachments.selectTypeRequired'));
    return;
  }
  ...
  message.success($t('seaExport.export.attachments.addOtherTypeSuccess'));
};

let cachedConfiguredTypes: SeaExportAdminApi.AttachmentDtlTypeSimpleDto[] = [];
let cachedAttachmentGroups: SeaExportAdminApi.AttachmentGroupDto[] = [];
...
const loadAttachments = async () => {
  if (!seaExportId.value) {
    message.warning($t('seaExport.export.attachments.noSeaExportId'));
    return;
  }
  ...
  try {
    ...
    const [moduleTypeResult, attachmentGroups, allTypes] = await Promise.all([
      ...
      getSeaExportAttachments(seaExportId.value),
      getAttachmentDtlTypeList(),
    ]);
    ...
    const configuredTypes =
      moduleTypeResult[0]?.attachmentDtlTypes?.filter(
        (item): item is SeaExportAdminApi.AttachmentDtlTypeSimpleDto =>
    ...
    console.error('加载海运出口附件失败:', error);
    message.error($t('seaExport.export.attachments.loadFailed'));
  } finally {
    loading.value = false;
  }
};
...
const handleBeforeUpload = async (
  ...
  if (!canEdit.value || !seaExportId.value) return false;
  ...
  try {
    ...
    await addSeaExportAttachments({
      id: seaExportId.value,
      attachments: [
        {
...
const handleDelete = (row: SeaExportAdminApi.AttachmentItemDto) => {
  if (!canEdit.value || !seaExportId.value) return;
  ...
  Modal.confirm({
    ...
    content: $t('seaExport.export.attachments.deleteConfirm', [fileName]),
    okText: $t('common.confirm'),
    cancelText: $t('common.cancel'),
    okType: 'danger',
    onOk: async () => {
      try {
        await deleteSeaExportAttachments({
          id: seaExportId.value,
        ...
        message.error($t('seaExport.export.attachments.deleteFailed'));
      }
    },
  });
};
...
<template>
  <div class="sea-export-attachments p-4">
        ...
        >
          ...
          <div class="attachment-card-list">
            ...
            >
              {{
                canEdit
                  ? $t('seaExport.export.attachments.clickOrDragUpload')
    ...
    <Modal
      ...
      :title="$t('seaExport.export.attachments.addOtherType')"
    ...
    >
      <div class="py-2">
        <div class="mb-2 text-sm text-gray-600">
          {{ $t('seaExport.export.attachments.addOtherTypeTip') }}
        </div>
        <Select
          v-model:value="selectedOtherTypeId"
          :options="otherTypeOptions"
          :placeholder="
            $t('seaExport.export.attachments.selectTypePlaceholder')
          "
          allow-clear
          class="w-full"
          show-search
