  ...
  try {
    ...
    const promises = gridRecords.map(async (row, index) => {
      ...
      if (
        ...
        row.closingTime !== originalDayData?.closingTime
      ) {
        modifiedFields.add('dateTimeMode');
      }

      // 检查星期模式是否变化
      const originalWeekDayData = originalRow.seFreiPriceWeekDays?.[0];
      if (
        row.etdDayOfWeek !== originalWeekDayData?.etdDayOfWeek ||
        row.etdDayTime !== originalWeekDayData?.etdDayTime ||
        row.closeDocDayOfWeek !== originalWeekDayData?.closeDocDayOfWeek ||
        row.closeDocDayTime !== originalWeekDayData?.closeDocDayTime ||
        row.closingDayOfWeek !== originalWeekDayData?.closingDayOfWeek ||
        row.closingDayTime !== originalWeekDayData?.closingDayTime
      ) {
      ...
      if (modifiedFields.has('dateTimeMode')) {
        const currentHasDateMode =
          row.etd || row.closeDocTime || row.closingTime;
        if (currentHasDateMode) {
          ...
          // 如果清空了所有日期字段，传空数组表示删除
          submitData.seFreiPriceDays = [];
        }
      }

      // 处理星期模式
      if (modifiedFields.has('weekMode')) {
        const currentHasWeekMode =
          row.etdDayOfWeek !== undefined ||
          row.closeDocDayOfWeek !== undefined ||
          row.closingDayOfWeek !== undefined;

        if (currentHasWeekMode) {
