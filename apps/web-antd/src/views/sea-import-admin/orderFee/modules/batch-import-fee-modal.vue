...
// 搜索表单
const searchForm = ref<OrderFeeAdminApi.SeaImportFeeQueryInputDto>({
  clientId: undefined,
  carrierId: undefined,
...
// 海运进口列表数据
const seaImportList = ref<OrderFeeAdminApi.SeaImportFeeListDto[]>([]);
const selectedSeaImport = ref<OrderFeeAdminApi.SeaImportFeeListDto | null>(
  null,
);
...
const handleSearch = async () => {
  loading.value = true;
  try {
    const params: OrderFeeAdminApi.SeaImportFeeQueryInputDto = {
      ...searchForm.value,
      paySide: currentPaySide.value, // 强制使用当前收付类型
    };

    const res = await getSeaImportFees(params);
    seaImportList.value = res || [];

    // 清空之前的选择
    selectedSeaImport.value = null;
    selectedFeeIds.value = [];

    // 如果查询结果不为空，自动选中第一条并加载其费用
    if (seaImportList.value.length > 0) {
      selectedSeaImport.value = seaImportList.value[0] || null;
      console.log(
        '✅ [handleSearch] 自动选中第一条业务:',
        selectedSeaImport.value,
      );
    }

    message.success({
      content: `查询到 ${seaImportList.value.length} 条符合条件的业务`,
      key: 'search_msg',
    });
  } catch (error) {
...
// 选择海运进口（单选）
const handleSelectSeaImport = (
  record: OrderFeeAdminApi.SeaImportFeeListDto,
) => {
  console.log('👆 [handleSelectSeaImport] 点击业务:', record);
  console.log(
    '📋 [handleSelectSeaImport] 该业务的费用数量:',
    record.orderFees?.length || 0,
  );

  selectedSeaImport.value = record;
  selectedFeeIds.value = []; // 清空已选费用

  console.log(
    '✅ [handleSelectSeaImport] 已选中业务，selectedSeaImport:',
    selectedSeaImport.value,
  );
};
...
<template>
  ...
  >
    <div class="batch-import-container">
      ...
      <!-- 海运进口列表 -->
      <div class="sea-import-section">
        <h3 class="section-title">
          海运进口业务列表 <span class="hint-text">（点击行选择）</span>
        </h3>
        <Table
          :dataSource="seaImportList"
          :columns="[
            {
              title: '委托编号',
          ...
          :rowClassName="
            (record) =>
              selectedSeaImport?.id === record.id ? 'selected-row' : ''
          "
          :customRow="
            (record) => ({
              onClick: () => handleSelectSeaImport(record),
            })
          "
          size="small"
        />
      ...
      <!-- 费用列表 -->
      <div class="fee-section" v-if="selectedSeaImport">
        <div class="fee-header">
          <h3 class="section-title">
            ...
            <span
              class="mbl-info"
              v-if="selectedSeaImport.transportOrder?.mblNum"
            >
              - 主提单号: {{ selectedSeaImport.transportOrder.mblNum }}
            </span>
          </h3>
          <!-- <Button
            type="primary"
        ...
        <Table
          :dataSource="selectedSeaImport.orderFees"
          :row-selection="{
