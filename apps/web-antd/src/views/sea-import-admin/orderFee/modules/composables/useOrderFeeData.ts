...
const extractSettlementNameMap = (
  orderDetail: SeaImportAdminApi.SeaImportDto | null | undefined,
): Map<any, string> => {
...
export function useOrderFeeData(
  ...
  // 订单基础数据
  const orderBaseData = ref<SeaImportAdminApi.SeaImportDto | null>(null);

  // 更改单 ID
  const changeOrderId = ref('');

  // 编辑 ID：KeepAlive 停用时冻结，避免跟上海出等其它 :id 页
  ...
  watch(
    ...
    async (newEditId, oldEditId) => {
      if (newEditId && newEditId !== oldEditId) {
        try {
          const orderDetail = await getSeaImportDetail(newEditId);
