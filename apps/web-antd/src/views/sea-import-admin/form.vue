...
const pageTitle = computed(() => {
  return isEdit.value
    ? $t('ui.actionTitle.edit', [$t('seaImport.import.name')])
...
const defaultOrderUsers: SeaImportAdminApi.OrderUserAddDto[] = [
...
const extractServiceTypesFromDetail = (
  ...
  collectServiceTypes((detail as any).seaImportServices);
...
const orderCtns = ref<SeaImportAdminApi.OrderCtnAddDto[]>([]);
...
const buildAiRecognizedFormValues = (payload: unknown) => {
  ...
  mergePayloadObject(basePayload.seaImport);
  mergePayloadObject(basePayload.transportOrderDto);
  mergePayloadObject(basePayload.seaImportDto);
...
const normalizeOrderCtnsWithRowKey = (
  items: SeaImportAdminApi.OrderCtnAddDto[] | undefined,
) => {
...
const ORDER_CTN_API_KEYS: Array<
  Extract<keyof SeaImportAdminApi.OrderCtnAddDto, string>
> = [
  'ctnCodeId',
  'ctnNo',
  'sealNo',
...
const sanitizeOrderCtns = (
  items: any[] | undefined,
): SeaImportAdminApi.OrderCtnAddDto[] => {
  if (!items?.length) return [];
  return items.map((item) => {
    ...
    return dto as SeaImportAdminApi.OrderCtnAddDto;
  });
};
                ...
                <div class="content-section__body">
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        {{ $t('seaImport.import.secondNotifierId') }}
                      </button>
                      <button
                        type="button"
                        class="transit-port-tabs__item"
                      ...
                      >
                        {{ $t('seaImport.import.overseasAgent') }}
                      </button>
                    </span>
                  </Teleport>
                </div>
                  ...
                  <UserSelect
                    ...
                    :selected-items="
                      row.userId
                        ...
                        : []
                    "
                    :placeholder="$t('seaImport.import.pleaseSelectOrderUser')"
                    size="small"
                    allow-clear
