...
import { hasValidUserId, toOptionalNumber } from './air-export-detail-mapper';

type OrderUserEditorRow = AirExportAdminApi.OrderUserAddDto & {
  _rowKey: string;
  userName?: string;
};
...
export function useOrderUsers(deps: UseOrderUsersDeps) {
   ...
   * 空运出口后端只硬性要求「销售恰好 1 个」，其余角色都可留空。
   */
  const requiredOrderUserRoles: number[] = [UserAttribute.Sales];
  const {
    roleLabelOf,
  ...
  const normalizeOrderUserItem = (item: AirExportAdminApi.OrderUserAddDto) => ({
    ...item,
  ...
  const createOrderUserRows = (
    items: AirExportAdminApi.OrderUserAddDto[] | undefined,
  ...
  const initializeOrderUsersPanel = (
    items?: AirExportAdminApi.OrderUserAddDto[],
    options?: { fillCurrentUser?: boolean },
  ) => {
