...
import type { ChangeOrderAdminApi } from '#/api/air-export/change-order-admin';
import {
  EditAsync,
  GetDetail,
  GetPagedList,
} from '#/api/air-export/change-order-admin';
...
/** 折叠态展示的前 10 个关键字段（适配器字段顺序即业务重要性排序） */
const KEY_ORDER_INFO_KEYS = airExportAdapter.displayFields
  .slice(0, 10)
  .map((field) => field.key);
...
const { displayFieldConfig } = useDisplayFieldConfig(
  airExportAdapter.displayFields,
  'order_fee_display_config',
);

/** 按适配器字段配置生成订单信息展示列表（取值差异收敛到适配器） */
const displayList = computed(() => {
  if (!formValues.value || !to.value) return [];

  return displayFieldConfig.value
    .filter((field) => field.visible)
    .map((field) => ({
      key: field.key,
      name: field.label,
      value: airExportAdapter.getDisplayValue(
        field.key,
        formValues.value,
        to.value,
      ),
        ...
        >
          ...
          >
            <span class="order-info-bar__title">
              ...
              {{ $t('airExport.export.formCardInfo') }}
            </span>
