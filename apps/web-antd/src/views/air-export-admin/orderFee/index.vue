...
const displayList = computed(() => {
  ...
  displayFieldConfig.value.forEach((field) => {
    ...
    // 根据 key 获取对应的值 - 调整为空运字段
    switch (field.key) {
      case 'mawbNum':
        value = to.value?.mawbNum || '--';
        break;
      case 'bookingNum':
      ...
      case 'teamName':
        ...
        break;
      case 'flightNo':
        value = formValues.value?.flightNo || '--';
        break;
      case 'carrierName':
        value =
...
const getOrderFeeNumber = async () => {
  ...
  // 查询应付费用
  params.PaySide = 1; // 应付
  const resPay = await getOrderFeePagedList(params);
      ...
      <div class="mx-2 flex items-stretch gap-6">
        ...
        <div class="flex min-w-0 flex-1 flex-col gap-2">
          ...
          <OrderFeeTable
            ...
            :order-detail="
              formValues as AirExportAdminApi.AirExportDto | undefined
            "
