...
const handleEdit = (row: FeeNameAdminApi.FeeNameDto) => {
  formModalApi.setData({ id: row.id }).open();
};

const handleDelete = async (row: FeeNameAdminApi.FeeNameDto) => {
...
const handleActionClick = ({
  code,
  row,
}: OnActionClickParams<FeeNameAdminApi.FeeNameDto>) => {
  switch (code) {
    case 'delete': {
      handleDelete(row);
      break;
...
const [Grid, gridApi] = useVbenVxeGrid<FeeNameAdminApi.FeeNameDto>({
  formOptions: {
...
<template>
  <Page auto-content-height>
    ...
    <Grid :table-title="$t('system.basicData.feeName.list')">
