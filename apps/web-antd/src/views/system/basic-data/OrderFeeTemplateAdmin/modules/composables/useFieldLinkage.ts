...
export function useFieldLinkage(dropdownSources: any) {
  ...
  const onFeeCodeChange = async (
    ...
    if (formApi) {
      try {
        ...
        console.log(
          '🔍 [onFeeCodeChange] 费用代码defaultCreditName:',
          feeDetail.defaultCreditName,
        );
        console.log(
          '🔍 [onFeeCodeChange] 费用代码defaultDebitName:',
          feeDetail.defaultDebitName,
        );
        ...
        if (paySide === 0) {
          ...
          console.log(
            '✅ [onFeeCodeChange] 应收模式，使用 defaultDebitName:',
          ...
          console.log(
            '✅ [onFeeCodeChange] 应付模式，使用 defaultCreditName:',
            industryCategoryValue,
          );
        }
        ...
        if (industryCategoryValue) {
          ...
          if (industryItem) {
            ...
            console.warn(
              '⚠️ [onFeeCodeChange] 未找到行业类别选项，value:',
              industryCategoryValue,
            );
          }
        } else {
  ...
  const onUnitPriceChange = (
    ...
    const effectiveTaxRate =
      taxRate !== null && taxRate !== undefined && !isNaN(taxRate)
        ? taxRate
        : 0;
