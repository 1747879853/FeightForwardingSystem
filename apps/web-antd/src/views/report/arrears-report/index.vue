...
// 导入表格组件（复用利润报表的表格组件）
import ProfitReportTable from '../profit-report/components/ProfitReportTable.vue';
...
// 权限检查 - 根据收付类型动态检查权限
const paySide = ref(0); // 默认应收
const canViewReceive = computed(() =>
  hasAccessByCodes(['Admin.Report.Arrears.Receive.Get']),
);
const canViewPay = computed(() =>
  hasAccessByCodes(['Admin.Report.Arrears.Pay.Get']),
);
const canView = computed(() => {
  if (paySide.value === 0) {
    return canViewReceive.value;
  } else {
    return canViewPay.value;
  }
});
...
async function handleQuery(formData?: any) {
  ...
  try {
    ...
    paySide.value = values.paySide || 0; // 更新收付类型

    // 如果没有权限，提示用户
    if (
      (values.paySide === 0 && !canViewReceive.value) ||
      (values.paySide === 1 && !canViewPay.value)
    ) {
...
function transformDataForHotTable(data: ReportApi.ArrearsReportDto[]) {
  ...
  return data.map((item) => {
    ...
    currencyCodes.forEach((code) => {
      ...
      rowData[`${code}_received`] = '';
      rowData[`${code}_unReceived`] = '';
    });
    ...
    if (item.currencies && item.currencies.length > 0) {
      item.currencies.forEach((curr) => {
        if (curr.currency?.code) {
          ...
          rowData[`${code}_received`] =
            (curr.received || 0).toFixed(2) === '0.00'
              ? ''
              : (curr.received || 0).toFixed(2);
          rowData[`${code}_unReceived`] =
            (curr.unReceived || 0).toFixed(2) === '0.00'
              ? ''
              : (curr.unReceived || 0).toFixed(2);
        }
      });
    }
...
<template>
  <Page class="arrears-report-page">
