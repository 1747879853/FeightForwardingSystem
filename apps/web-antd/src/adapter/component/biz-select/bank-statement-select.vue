...
interface Props {
  /** label 字段名，默认 'bankStatementNo' */
  labelKey?: string;
  /** 每页数量，默认 20 */
  pageSize?: number;
  /** placeholder */
  placeholder?: string;
  /** 已选中的银行流水对象数组（用于编辑时回显） */
  selectedItems?: BankStatementAdminApi.BankStatementListDto[];
  /** value 字段名，默认 'id' */
  valueKey?: string;
}

const props = withDefaults(defineProps<Props>(), {
  labelKey: 'bankStatementNo',
  pageSize: 20,
...
const mapBankStatementToOption = (
  item: BankStatementAdminApi.BankStatementListDto,
) => {
  const itemAny = item as any;
  const label =
    itemAny?.[props.labelKey] || item.bankStatementNo || item.id || '';
