...
interface Props {
  /** label 字段名，默认 'countryName'，可用值：'countryName' | 'countryEnName' | 'code' */
  labelKey?: string;
  /** 每页数量，默认 20 */
  pageSize?: number;
  /** placeholder */
  placeholder?: string;
  /** 已选中的国家对象数组（用于编辑时回显） */
  selectedItems?: CountryCodeAdminApi.CountryCodeDto[];
  /** value 字段名，默认 'id' */
  valueKey?: string;
}
...
const mapCountryToOption = (country: CountryCodeAdminApi.CountryCodeDto) => {
  const countryAny = country as any;
  let label = countryAny?.[props.labelKey];
  if (!label && props.labelKey === 'countryEnName') {
...
const fetchPageAdapter = async (params: {
  ...
  const res = await getCountryCodePagedList({
