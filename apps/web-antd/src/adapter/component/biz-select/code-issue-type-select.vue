...
interface Props {
  /** label 字段名，默认 'billType'，可用值：'billType' | 'enName' */
  labelKey?: string;
  /** 每页数量，默认 20 */
  pageSize?: number;
  /** placeholder */
  placeholder?: string;
  /** 已选中的签单方式对象数组（用于编辑时回显） */
  selectedItems?: CodeIssueTypeAdminApi.CodeIssueTypeDto[];
  /** value 字段名，默认 'id' */
  valueKey?: string;
}
...
const mapItemToOption = (item: CodeIssueTypeAdminApi.CodeIssueTypeDto) => {
  const itemAny = item as any;
  let label = itemAny?.[props.labelKey];
  if (!label && props.labelKey === 'enName') {
    label = item.billType;
  }
  label = label || item.billType || item.enName || '';
...
const fetchPageAdapter = async (params: {
  ...
  const res = await getCodeIssueTypePagedList({
...
const ensureSelectedLoaded = async (rawValue: any) => {
  ...
  for (const v of values) {
    ...
    loadedSelectedIds.value.add(idStr);
    try {
      const detail = await getCodeIssueTypeDetail(idStr);
