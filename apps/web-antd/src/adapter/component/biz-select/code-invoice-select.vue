...
interface Props {
  /** label 字段名，默认 'name'，可用值：'name' | 'code' */
  labelKey?: string;
  /** 每页数量，默认 20 */
  pageSize?: number;
  /** placeholder */
  placeholder?: string;
  /** 已选中的发票商品编码对象数组（用于编辑时回显） */
  selectedItems?: CodeInvoiceAdminApi.CodeInvoiceDto[];
  /** value 字段名，默认 'id' */
  valueKey?: string;
}
...
const mapItemToOption = (item: CodeInvoiceAdminApi.CodeInvoiceDto) => {
  const itemAny = item as any;
  let label = itemAny?.[props.labelKey];
  if (!label && props.labelKey === 'code') {
    label = item.name;
  }
  label = label || item.name || item.code || '';
...
const fetchPageAdapter = async (params: {
  ...
  const res = await getCodeInvoicePagedList({
...
const ensureSelectedLoaded = async (rawValue: any) => {
  ...
  for (const v of values) {
    ...
    loadedSelectedIds.value.add(idStr);
    try {
      const detail = await getCodeInvoiceDetail(idStr);
