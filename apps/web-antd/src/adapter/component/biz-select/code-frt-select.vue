...
interface Props {
  ...
  /** placeholder */
  placeholder?: string;
  /** 已选中的付费方式对象数组（用于编辑时回显） */
  selectedItems?: CodeFrtAdminApi.CodeFrtDto[];
  /** value 字段名，默认 'id' */
  valueKey?: string;
}
...
const mapItemToOption = (item: CodeFrtAdminApi.CodeFrtDto) => {
...
const fetchPageAdapter = async (params: {
  ...
  const res = await getCodeFrtPagedList({
...
const ensureSelectedLoaded = async (rawValue: any) => {
  ...
  for (const v of values) {
    ...
    loadedSelectedIds.value.add(idStr);
    try {
      const detail = await getCodeFrtDetail(idStr);
