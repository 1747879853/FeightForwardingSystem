...
interface Props {
  ...
  /** placeholder */
  placeholder?: string;
  /** 已选中的业务来源对象数组（用于编辑时回显） */
  selectedItems?: CodeSourceAdminApi.CodeSourceDto[];
  /** value 字段名，默认 'id' */
  valueKey?: string;
}
...
const mapItemToOption = (item: CodeSourceAdminApi.CodeSourceDto) => {
...
const fetchPageAdapter = async (params: {
  ...
  const res = await getCodeSourcePagedList({
...
const ensureSelectedLoaded = async (rawValue: any) => {
  ...
  for (const v of values) {
    ...
    loadedSelectedIds.value.add(idStr);
    try {
      const detail = await getCodeSourceDetail(idStr);
