...
interface Props {
  ...
  placeholder?: string;
  /** 已选中的费用对象数组（用于编辑时回显） */
  selectedItems?: FeeNameAdminApi.FeeNameDto[];
  /** value 字段名，默认 'id' */
  valueKey?: string;
}
...
const mapItemToOption = (item: FeeNameAdminApi.FeeNameDto) => {
...
const fetchPageAdapter = async (params: {
  ...
  const res = await getFeeNamePagedList({
...
const ensureSelectedLoaded = async (rawValue: any) => {
  ...
  for (const v of values) {
    ...
    loadedSelectedIds.value.add(idStr);
    try {
      const detail = await getFeeNameDetail(idStr);
