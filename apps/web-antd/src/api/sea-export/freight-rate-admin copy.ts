 ...
 * 费用代码简要信息
 */
export interface SeFreiPriceFeeCodeSimpleDto {
  /** 费用ID */
  feeCodeId: number;
  /** 费用代码 */
  feeCodeCode: string;
 ...
 * 运价箱型费用（新增/编辑用）
 */
export interface SeFreiPriceCtnFeeInput {
  /** 子表主键ID（编辑时有值，新增时为空） */
  id?: string;
  /** 费用ID */
  feeCodeId: number;
 ...
 * 运价箱型报价（新增/编辑用）
 */
export interface SeFreiPriceCtnInput {
  /** 子表主键ID（编辑时有值，新增时为空） */
  id?: string;
  /** 箱型ID */
  ctnCodeId: number;
  ...
  remark?: string;
  /** 运价箱型费用列表 */
  seFreiPriceCtnFees?: SeFreiPriceCtnFeeInput[];
}
 ...
 * 运价详情（输出）
 */
export interface SeFreiPriceDetailDto {
  /** 运价主键ID */
  id: string;
  /** 是否推荐 */
  recommend: boolean;
  ...
  /** 费用代码简要信息列表 */
  feeCodeInfos?: SeFreiPriceFeeCodeSimpleDto[];
}

/**
 * 运价列表项（输出）
 */
export interface SeFreiPriceListItemDto extends Omit<
  SeFreiPriceDetailDto,
  'seFreiPriceCtns'
> {
  /** 箱型报价列表 */
  seFreiPriceCtns?: SeFreiPriceCtnOutDto[];
...
export interface AddSeFreiPriceInput {
  ...
  remark?: string;
  /** 箱型报价列表 */
  seFreiPriceCtns?: SeFreiPriceCtnInput[];
}
...
export interface BatchEditSeFreiPriceInput {
  ...
  /** 备注（为null不修改） */
  remark?: string;
  /** 箱型报价列表（为null不修改；不为null则删除所有原有子表重新添加） */
  seFreiPriceCtns?: SeFreiPriceCtnInput[];
}
...
export function getSeFreiPriceDetail(id: string) {
  return requestClient.get<SeFreiPriceDetailDto>(`${BASE_URL}/DetailAsync`, {
    params: { id },
  });
}
...
export function getSeFreiPriceList(params: GetSeFreiPriceListInput) {
  return requestClient.get<SeFreiPriceListItemDto>(
    `${BASE_URL}/GetPagedListAsync`,
    { params },
  );
}
...
export function getAllLaneCodes() {
  return requestClient.get<{ laneCodes: LaneCodeDto[] }>(
